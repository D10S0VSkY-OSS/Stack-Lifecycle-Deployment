...
from helpers.get_data import user_squad_scope, activity_log, check_squad_user, check_role_user
...
async def create_user(
    ...
    if not crud_users.is_master(db, current_user):
        if not check_squad_user(current_user.squad, user.squad):
            raise HTTPException(status_code=403, detail=f"Not enough permissions for some of these squads {user.squad}")
        if not check_role_user(current_user.role, user.role):
            raise HTTPException(status_code=403, detail=f"Not enough permissions for some of these roles {user.role}")
