...
def test_list_users():
    response = request_url(verb='GET', uri='users/?limit=1', headers={
                           "Authorization": f"Bearer {token}"})
    assert response.get('status_code') == 200
...
def test_delete_user_test():
    list_user = request_url(verb='GET', uri=f'users/{user_test}', headers={
        "Authorization": f"Bearer {token}"})
    id = list_user.get("json").get("id")
    response = request_url(verb='DELETE', uri=f'users/{id}', headers={
                           "Authorization": f"Bearer {token}"})
    assert response.get('status_code') == 200
