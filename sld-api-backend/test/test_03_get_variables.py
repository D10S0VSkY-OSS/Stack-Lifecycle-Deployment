...
def test_variables_stack_list_by_id():
    response = request_url(verb='GET', uri=f'stacks/{stack_name}', headers={
...
def test_variables_stack_list_by_name():
    response = request_url(
        verb='GET', uri=f'variables/list?stack={stack_name}', headers={
            "Authorization": f"Bearer {token}"})
    result = response.get('status_code')
    assert result == 200
...
def test_variables_stack_json_by_name():
    response = request_url(
        verb='GET', uri=f'variables/json?stack={stack_name}', headers={
            "Authorization": f"Bearer {token}"})
    result = response.get('status_code')
    assert result == 200
