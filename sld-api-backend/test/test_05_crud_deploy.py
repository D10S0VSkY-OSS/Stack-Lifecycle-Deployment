...
def test_deploy_stack_stormtrooper():
    response = request_url(verb='POST', uri=f'deploy/{uri}', headers={
                           "Authorization": f"Bearer {token_unprivileges}"}, json=data_user)
    if response.get('status_code') != 409:
        task_id = response.get('json').get('task').get('task_id')
        while get_task_id(task_id).get('json').get(
...
def test_deploy_stack_darth_vader():
    response = request_url(verb='POST', uri=f'deploy/{uri}', headers={
                           "Authorization": f"Bearer {token_user_squad}"}, json=data)
    if response.get('status_code') != 409:
        task_id = response.get('json').get('task').get('task_id')
        while get_task_id(task_id).get('json').get(
                'result').get('state') != 'SUCCESS':
...
def test_get_deploy_by_id():
    response = request_url(verb='GET', uri='deploy/?limit=1', headers={
                           "Authorization": f"Bearer {token}"})
...
def test_get_all_tasks():
    response = request_url(verb='GET', uri='tasks/all?limit=100', headers={
                           "Authorization": f"Bearer {token}"})
    result = response.get('status_code')
    assert result == 200
...
def test_destroy_stack():
    response = request_url(verb='GET', uri='deploy/?limit=1', headers={
                           "Authorization": f"Bearer {token}"})
    deploy_id = response.get('json')[0].get("id")
    response = request_url(verb='DELETE', uri=f'deploy/{deploy_id}', headers={
                           "Authorization": f"Bearer {token}"})
    task_id = response.get('json').get('task').get('task_id')
    while get_task_id(task_id).get('json').get(
            'result').get('state') != 'SUCCESS':
